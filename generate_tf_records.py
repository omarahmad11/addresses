...
def generate_tf_records(input_file_path: str, output_file_path: str, input_gzip: bool=True):
    ...
    with file_open(input_file_path, file_open_mode, newline="") as f:
        ...
        string_fields = ('Building_name', 'Block', 'Level', 'Unit','House_number' ,'Street_name','Township','Locality_name','State','Postcode' )
...
if __name__ == "__main__":
    ...
    #parser.add_argument("ASTROData.csv", help="CSV file exported from GNAF `address_view`")
    #parser.add_argument("sample_record_output", help="Path to tfrecords output")
    #parser.add_argument("--gzipped_input", action="store_true", default=False)
    #args = parser.parse_args()

    print("Generating tfrecords files...")
    generate_tf_records("ASTROData.csv", "sample2_record_output.tfrecord", False)
    print("Done!")